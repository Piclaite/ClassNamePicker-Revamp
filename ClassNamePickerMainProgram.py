...
class PickName(QMainWindow, Ui_MainWindow, QWidget):
    ...
    def _load_student_data(self) -> None:
        ...
        self.no_duplicate_cache = self.no_duplicate  # 同步到缓存，以便切换时恢复
        ...
        while retry_count <= MAX_RETRIES:
            try:
                ...
                female_names = {s.name for s in female_students}
                ...
                if self.no_duplicate >= total:
                    ...
                    self.no_duplicate_cache = self.no_duplicate  # 同步到缓存
    ...
    def _restore_application_config(self) -> None:
        ...
        try:
            ...
            saved_picked_names = set(config.get(ConfigManager.KEY_SAVED_PICKED_NAMES, []))
            #if self.is_saving_results and saved_names:
            if self.is_saving_results and saved_picked_names:
                #self.student_pool.restore_available_names(saved_names)
                self.student_pool.restore_picked_names(saved_picked_names)
                #print(f"[恢复] 已恢复{len(saved_names)}个可用名字")
                print(f"[恢复] 已恢复 {len(saved_picked_names)} 个已抽取记录")
            else:
                print("[恢复] 无历史抽取记录，使用默认全可用状态")
        except Exception as e:
            print(f"[恢复失败]: {e}")
        # 更新统计显示
    ...
    def _flush_all_saves(self):
        ...
        if {'state', 'floating'}.intersection(reasons_to_process):
            ...
            new_ui_state = {
                ...
                ConfigManager.KEY_NO_DUPLICATE: self.no_duplicate_cache,
        ...
        if hasattr(self, 'student_pool') and self.is_saving_results:
            ...
            picked_names = list(self.student_pool.get_picked_names())
            if set(config.get(ConfigManager.KEY_SAVED_PICKED_NAMES, [])) != set(picked_names):
                updates[ConfigManager.KEY_SAVED_PICKED_NAMES] = picked_names
                print(f"[SAVE] 已抽取名单已更新，共{len(picked_names)}人")
    ...
    def _rebuild_student_pool(self):
        ...
        current_picked = old_pool.get_picked_names()
        ...
        self.student_pool.restore_picked_names(current_picked)
        print(f"[重建] 已恢复 {len(current_picked)} 个已抽取记录")
    ...
    def _on_toggle_repeat(self):
        ...
        if is_checked:
            ...
            self.no_duplicate = 0
            print(f"[DATA] 防重复设置: {self.no_duplicate}, 缓存: {self.no_duplicate_cache}")
    ...
    def _update_animation(self):
        ...
        if elapsed < self.animation_time:
            ...
            rdm_name = self.student_pool._students[random_id].name
    ...
    def _display_result(self, display_name: str):
        ...
        # 异步语音播报（先应用多音字替换：命中配置时播报发音字）
        if self.is_speech_enabled:
            speak_text = self._apply_name_changes(display_name)
            self._speak_name_async(speak_text)
    ...
    def _apply_name_changes(self, name: str) -> str:
        """应用多音字替换（带缓存）：命中配置则返回发音字，否则返回原名"""
        # 文件变更检测：name_changes.json 被修改（配置面板保存等）时自动刷新缓存，避免使用旧快照
        try:
            mtime = ConfigManager.NAME_CHANGES_FILE.stat().st_mtime
            if self.__dict__.get('_name_changes_mtime') != mtime:
                self._name_changes_cache = ConfigManager.load_name_changes()
                self._name_changes_mtime = mtime
        except OSError:
            pass
        # 兜底：属性不存在时直接加载
        if '_name_changes_cache' not in self.__dict__:
            self._name_changes_cache = ConfigManager.load_name_changes()
        # 遍历三组多音字配置（最多三组）
        for c in 'abc':
            orig = self._name_changes_cache.get(f'speak_change_{c}1', '').strip()
            if orig and name == orig:
                replaced = self._name_changes_cache.get(f'speak_change_{c}2', '').strip()
    ...
    def _on_config_applied(self, new_config: dict):
        ...
        self.no_duplicate_cache = self.no_duplicate  # 同步缓存
        ...
        # 刷新多音字替换缓存（配置面板可能保存了新发音字）
        self._name_changes_cache = ConfigManager.load_name_changes()
        try:
            self._name_changes_mtime = ConfigManager.NAME_CHANGES_FILE.stat().st_mtime
        except OSError:
            pass
